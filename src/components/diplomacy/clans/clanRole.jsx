...
export default function ClanRole({ member, role, clan })
{
    ...
    return (
        <section>
            <Text>{t('currentRole')}: {role}</Text>
            {permission &&
                <Group p='sm'>
                    {role === 'member' ? (<Button onClick={promote}>{t('diplomacy:clan.promote')}</Button>) : null}
                    {role === 'Assistant' ? (<Button onClick={demote} color='red'>{t('diplomacy:clan.demote')}</Button>) : null}
                    {member.id !== empire.id && <Button onClick={remove} color='red'>{t('diplomacy:clan.remove')}</Button>}
                </Group>}
        </section>
    )
}