...
export default function JoinClan({ disabled }) {
	...
	const SelectItem = forwardRef(({ name, value, ...others }, ref) => (
		<div ref={ref} {...others}>
			<div>
	...
	const joinClan = async (values) => {
		try {
			const res = await Axios.post(
				`/clans/join?gameId=${empire.game_id}&lang=${i18n.language}`,
				values,
			)
			// console.log(res)
			showNotification({
	...
	return (
		<Paper maw={400} p={20}>
			...
			{clans.length > 0 ? (
					...
					<Card>
						{disabled && (
							<Text align="center" color="red">
								{t("diplomacy:clans.joinDisabled")}
							</Text>
						)}
						<Select
							searchable
				...
				<Text align="center">{t("diplomacy:clans.noClans")}</Text>
			)}
		</Paper>
	)
}
