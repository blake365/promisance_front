		...
		<Card shadow="sm" radius="sm" m="sm" p="xs" withBorder>
			<Card.Section p="xs">
				<Group position="apart">
					...
					{!item.seen && <Badge color="green">{t("diplomacy:news.new")}</Badge>}
				</Group>
			</Card.Section>
			<Group noWrap spacing="xs">
				<Box>{selectIcon(item.type, item.result)}</Box>
				<Box>
					<Text size="sm" weight="bold">
						{t(getContentKey(item.privateNews), item.privateNews?.params || {})}
					</Text>
				</Box>
			</Group>
		</Card>
